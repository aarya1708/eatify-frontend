...
export default function Order() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.post("http://localhost:9000/rest-order", {
          action: "restaurant",
  ...
  const updateOrderStatus = async (id) => {
    try {
      const response = await axios.patch("http://localhost:9000/rest-order", {
        restaurantName: restaurantName,
        id: id,
      });
      ...
      if (response.status === 200) {
        ...
        axios.post("http://localhost:9000/rest-order", {
          action: "delivery",
          order: deliveryOrder
        })
  ...
  const HandleDelivery = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:9000/rest-order`, {
        data: { restaurantName: restaurantName, id: orderToMove.id },
      });

      const response2 = await axios.post("http://localhost:9000/rest-order", {
        action: "prevOrder",
        order: orderToMove
      });
