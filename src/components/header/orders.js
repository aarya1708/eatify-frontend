...
const PreviousOrders = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.post('http://localhost:9000/user-prev-order', { email: decodedToken.email });
        let fetchedPreviousOrders = response.data.previousOrders;

        const response2 = await axios.post('http://localhost:9000/user-curr-order', { email: decodedToken.email });
