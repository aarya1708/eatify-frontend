...
function Ufc() {
    ...
    useEffect(() => {
        ...
        axios.post("https://eatify-frontend.vercel.app//United%20Farmers%20Creamery", {action:'user', email: "ufc@gmail.com" })  
            .then((response) => {
    ...
    menu.map((item) => (
        <>        
        <Menuitem key={item.name} cart={cart} setCart={setCart} item={item} restaurant="United Farmers Creamery" />
        </>
    ))
) : (
    <p>Loading menu...</p>
