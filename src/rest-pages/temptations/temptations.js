...
function Temptations() {
    ...
    useEffect(() => {
        ...
        axios.post("http://localhost:9000/Temptations", { action:'user', email: "temptations@gmail.com" }) 
            .then((response) => {
    ...
    menu.map((item) => (
        <>        
        <Menuitem key={item.name} cart={cart} setCart={setCart} item={item} restaurant="Temptations" />
        </>
    ))
) : (
    <p>Loading menu...</p>
