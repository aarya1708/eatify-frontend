...
function Kdh() {
    ...
    useEffect(() => {
        ...
        axios.post("https://eatify-frontend.vercel.app//Kakke%20Di%20Hatti", {action:'user', email: "kakkedihatti@gmail.com" })  
            .then((response) => {
    ...
    menu.map((item) => (
        <>        
        <Menuitem key={item.name} cart={cart} setCart={setCart} item={item} restaurant="Kakke Di Hatti" />
        </>
    ))
) : (
    <p>Loading menu...</p>
