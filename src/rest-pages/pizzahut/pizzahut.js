...
function Pizzahut() {
    ...
    useEffect(() => {
        ...
        axios.post("https://eatify-frontend.vercel.app//Pizza%20Hut", { action:'user', email: "pizzahut@gmail.com" })  
            .then((response) => {
    ...
    menu.map((item) => (
        <>        
        <Menuitem key={item.name} cart={cart} setCart={setCart} item={item} restaurant="Pizza Hut" />
        </>
    ))
) : (
    <p>Loading menu...</p>
